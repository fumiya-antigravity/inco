...
const WikiView = () => {
 ...
 return (
 <div className="max-w-6xl mx-auto w-full animate-in fade-in duration-300 h-full flex flex-col bg-white shadow-sm border border-slate-200 rounded-xl my-4 overflow-hidden">
 {/* Header */}
 <div className="flex justify-between items-center py-4 px-6 border-b border-slate-200 bg-slate-50/50">
 ...
 {/* Wiki Sidebar */}
 <div className="w-64 border-r border-slate-200 bg-slate-50/30 flex flex-col">
 ...
 : 'text-slate-600 hover:bg-slate-100:bg-zinc-800'
 }
 `}
 >
 <FileQuestion size={14} className={selectedPageId === page.id ? 'text-emerald-600' : 'text-slate-400'} />
 ...
 className="text-slate-400 hover:text-emerald-600:text-emerald-400 p-2 rounded-lg hover:bg-slate-100:bg-zinc-800 transition-all flex items-center gap-1.5 text-sm font-bold"
 title="編集"
 >
 <Edit2 size={16} /> 編集
 </button>
 ...
 className="text-slate-500 hover:text-slate-700:text-slate-200 px-3 py-1.5 rounded-lg hover:bg-slate-100:bg-zinc-800 transition-all text-sm font-bold flex items-center gap-1"
 >
 <X size={16} /> キャンセル
 </button>
 <button
 ...
 className="w-full text-3xl font-bold text-slate-800 border-b border-transparent focus:border-emerald-500 focus:outline-none bg-transparent py-2 placeholder-slate-300"
 placeholder="ページタイトル"
 />
 <textarea
 value={editContent}
 onChange={(e) => setEditContent(e.target.value)}
 className="w-full h-[60vh] resize-none text-slate-700 leading-relaxed border-none focus:outline-none bg-slate-50 rounded-xl p-6 text-base font-mono"
 ...
 <h1 className="text-3xl font-bold text-slate-800 mb-6 pb-4 border-b border-slate-100">
 {selectedPage.title}
 </h1>
