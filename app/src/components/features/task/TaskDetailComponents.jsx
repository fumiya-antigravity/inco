...
export const DetailRow = ({ icon: Icon, label, hasValue, onClear, children, onClick, onAdd }) => (
 <>
 <div className="text-slate-500 flex items-center gap-2 text-sm h-9">
 <Icon size={18} /> {label}
 </div>
 <div
 className={`
 ...
 ${hasValue
 ? 'bg-slate-100/80 hover:bg-slate-200/80:bg-zinc-700'
 : 'bg-transparent hover:bg-slate-100:bg-zinc-800 text-slate-400 border-dashed hover:border-slate-300'
 }
 `}
 onClick={onClick}
 >
 <div className={`flex items-center gap-2 truncate ${hasValue ? 'text-slate-700 font-medium' : 'text-slate-400'}`}>
 {children}
 </div>
 ...
 className="p-1 -mr-1 rounded-full hover:bg-slate-300/50:bg-slate-600 text-slate-400 hover:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity"
 title="追加"
 >
 <Plus size={14} />
 </button>
 ...
 className="p-1 rounded-full hover:bg-slate-200:bg-zinc-700 text-slate-400 hover:text-slate-600"
 title="クリア"
 >
 <X size={14} />
 </button>
...
export const SubtaskItem = ({ subtask, onToggle, onDelete }) => (
 <div className="group flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50:bg-zinc-800/50 transition-colors">
 <button
...
export const SubtaskInput = ({ onAdd, onCancel }) => {
 ...
 <div className="w-5 h-5 rounded border border-dashed border-slate-300 flex-shrink-0"></div>
 <input
 autoFocus
 type="text"
 value={title}
...
export const ActivityItem = ({ activity, onEdit, onDelete }) => {
 ...
 return (
 <div className="flex gap-3 text-xs text-slate-400 pl-2 border-l-2 border-slate-100 ml-3 py-1">
 ...
 <span className="font-bold text-sm text-slate-700">{activity.user || 'Unknown'}</span>
 ...
 className="w-full bg-transparent border-none outline-none text-sm text-slate-700 resize-none min-h-[60px]"
 />
 ...
 <div className="text-sm text-slate-700 leading-relaxed bg-slate-50 p-3 rounded-lg rounded-tl-none">
 {activity.text}
 </div>
 )}
...
export const CommentInput = ({ onPost }) => {
 ...
 <div className="flex-1 border border-slate-200 rounded-xl bg-white focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all overflow-hidden">
 <textarea
 value={text}
 onChange={(e) => setText(e.target.value)}
 className="w-full bg-transparent border-none outline-none text-sm p-3 min-h-[80px] resize-none text-slate-700"
 placeholder="コメントを入力..."
 />
 <div className="flex justify-between items-center bg-slate-50 px-3 py-2 border-t border-slate-100">
 <div className="text-xs text-slate-400">Markdown対応</div>
 <button
 onClick={handlePost}
 disabled={!text.trim()}
 className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all
 ${text.trim() ? 'bg-emerald-500 text-white hover:bg-emerald-600 shadow-sm' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}
 `}
 >
 <Send size={12} /> コメント
 </button>
