            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        className={`w-full px-3 py-2 bg-slate-50 dark:bg-zinc-800 border rounded-lg focus:outline-none focus:ring-2 transition-all font-mono uppercase dark:text-slate-100 
                            ${error ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500' : 'border-slate-200 dark:border-zinc-700 focus:ring-emerald-500/20 focus:border-emerald-500'}`}
                        placeholder="EX: WEB"
                        maxLength={6}
                    />
                    {error ? (
                            ...
                            {icons.map(iconName => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all border ${selectedIcon === iconName ? 'bg-slate-100 dark:bg-zinc-700 border-emerald-500 text-emerald-500' : 'bg-transparent border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50 dark:hover:bg-zinc-800'}`}
                                    >
                                        <Icon size={18} />
                                    </button>
                                );
                ...
                {/* Preview */}
                <div className="bg-slate-50 dark:bg-zinc-900 rounded-lg p-4 flex items-center gap-4 border border-slate-100 dark:border-zinc-800">
                    <ProjectIcon color={selectedColor} icon={selectedIcon} size="lg" />
                    <div>
                        <div className="font-bold text-slate-800 dark:text-slate-200">{name || 'プロジェクト名'}</div>
