...
*   **プロジェクト (Project)**:
    *   **初期値**: タスク作成時は作成元のプロジェクトが自動設定される。
    *   **挙動**:
        *   タグ形式で表示され、複数のプロジェクトを紐付け可能。
        *   「追加」ボタンからドロップダウンでプロジェクトを選択して追加。
        *   各タグの「×」ボタンで解除可能（ただし最低1つの紐付けが必須）。
*   **担当者 (Assignee)**:
    *   **初期値**: `未割当` (Unassigned)
...
*   **セクション (Section)**:
    *   **初期値**: タスク作成時のコンテキスト（リストのセクション等）に基づく。
    *   **挙動**: 
        *   ステータスと同様のボタンスタイルで表示（色付き）。
        *   単一選択のみ可能。
        *   ドロップダウンからプロジェクト内のセクションを選択。
*   **ステータス (Status)**:
    *   **初期値**: `未対応` (Not Started)
    *   **選択肢**: `未対応`, `処理中`, `完了`
*   **優先度 (Priority)**:
