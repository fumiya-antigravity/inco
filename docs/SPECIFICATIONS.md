...
### リストビュー (ListView)
*   **インライン編集**:
    *   タスクの「件名」をクリックすると、その場で入力モード（テキストボックス）に切り替わる。
    *   `Enter` キーで保存、`Esc` キーまたはフォーカスアウトで編集を終了する。
*   **パネル展開の制御**:
...
*   **開閉挙動**:
    ...
    *   パネルの外側（バックドロップ領域）をクリックした場合、パネルを閉じる（URLパラメータを削除する）。
